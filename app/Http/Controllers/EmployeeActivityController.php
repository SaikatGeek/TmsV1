...
class EmployeeActivityController extends Controller
{
	...
	// 			$currentTime[$index] = new DateTime($value->created_at);
	// 		}
	// 		elseif($value->status == 'OFF_DESK'){
	// 			$currentTime2[$index-1] = new DateTime($value->created_at);//end time
	// 			// $interval[$ind] = $currentTime->diff($currentTime2);

	// 			dump($currentTime2);

	// 		}
	// 		// elseif($value->status == 'ON_DESK'){
	// 		// 	$onDesk = new DateTime('2016-11-30 11:55:06');//start time
	// 		// }
			
	// 		// $sum = $currentTime[$index]->diff($currentTime2[$index]);
	...
	public function __productiveTime($activity_id){
		...
		// 	if($value->status == 'ON_DESK'){
		// 		$onDesk = Carbon::parse($value->created_at);
		// 	}
		// 	elseif($value->status == 'OFF_DESK'){
		// 		$offDesk = Carbon::parse($value->created_at;
		// 	}			
		// }
	...
	public function __newActivity($info){
	  ...
	  if($newRow->save()){
	  	if($info['status'] == "DESK_OPEN"){
	  		...
	  		$reference = "/employee/single/workday/{$info['employeeActivityId']}";
	  		$entity = "DESK_OPEN";
