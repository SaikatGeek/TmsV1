...
class ProjectController extends Controller
{
    ...
    public function myAcceptedTaskPage(){
        ...
        if($auth->type == 1){
            $TaskList = Task::where('status', 'Accepted')->orderBy('id', 'desc')->get();
        }
        else{
            $TaskList = Task::where('user_id', $id)->where('status', 'Accepted')->orderBy('id', 'desc')->get();
        }        
    ...
    public function myProjectDetailsPage($project_id){
        ...
        foreach ($ProjectMember as $key => $value) {
            $value->totalTask = Task::where('project_id', $project_id)->where('user_id', $value->user_id )->count();
        }
    ...
    public function readNotifications(){
        ...
        foreach ($NotificationList as $key => $value) {
          $value->image = User::find($value->notified_by)->image;
        }
    ...
    public function ajaxOngoingTask(){
      //  Task::orderByRaw("FIELD(status,  \"Submitted\", \"Resubmitted\", \"In Process...\", \"In Review\", \"In Revision\", \"Revision Needed\", \"Tested\")")
      //               ->get();

      $TaskList = DB::select('select * from tasks where status = ? OR status = ? OR status = ? OR status = ? OR status = ? OR status = ? OR status = ? ORDER BY status DESC', ['Submitted', 'Resubmitted', 'In Process...', 'In Review', 'In Revision', 'Revision Needed', 'Tested']);
        
      foreach($TaskList as $index => $item){
        $item->projectName  = Projects::find($item->project_id)->title;
        $item->developer  = User::find($item->user_id)->name;
        $item->createdBy  = User::find($item->created_by)->name;
        $item->assignedDate  = date("d/m/Y", strtotime($item->created_at));
        $item->submissionDate  = date("d/m/Y", strtotime($item->submission_date));
        $item->submissionTime  = date("h:i A", strtotime($item->submission_time));
        $item->latestStatusTime  =  date("d/m/Y h:i A", strtotime($item->updated_at));
      }
